...
ssh $SSH_HOST MONGO_URL=$MONGO_URL ROOT_URL=$ROOT_URL APP_DIR=$APP_DIR APP_NAME=$APP_NAME NODE_ENV=production 'bash -s' <<'ENDSSH'
...
rm /tmp/bundle.tgz
PORT=3001 nohup "node" "bundle/main.js" 1>>"$APP_NAME.log" 2>"$APP_NAME.log" & echo $! > $APP_NAME.pid
popd
ENDSSH
